...
class UpdateDialog(QDialog):
    def __init__(self, url, title, content, necessary=False):
        ...
        self.ui.pushButton_download.clicked.connect(lambda: (QDesktopServices.openUrl(QUrl(url)), exit()))
...
def check_update():
    ...
    try:
        ...
        dialog = UpdateDialog('https://blog.csdn.net/Yang_shengzhou', '网络连接失败', '内测版本,必须连接互联网使用,感谢您的理解', True)
        dialog.exec()
